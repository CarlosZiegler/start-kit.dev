import type { ThemeConfig, ThemeCssVars } from "./theme-types";
import {
  BASE_COLOR_NAMES,
  FONT_OPTIONS,
  RADIUS_VALUES,
...
const TEMPLATE_UTILITIES = `@layer utilities {
  @keyframes tech-marquee-scroll {
    from {
      transform: translateX(0);
    }
    to {
      transform: translateX(calc(-100% - var(--tech-marquee-gap, 2.5rem)));
    }
  }

  .tech-marquee {
    --tech-marquee-gap: 2.5rem;
    --tech-marquee-duration: 40000ms;
  }

  .tech-marquee__group {
    animation: tech-marquee-scroll var(--tech-marquee-duration) linear infinite;
  }

  .tech-marquee:hover .tech-marquee__group,
  .tech-marquee:focus-within .tech-marquee__group {
